...
import { Check, ChevronDown, Search } from "../../storybook/Foundation/Icons/Icons";
...
const SelectNew = ({
    ...
    const clickingInsideSelectBox = (e: any) => {
        ...
        if (selectBox?.contains(e.target) && !clearBtn?.contains(e.target) && !dropdownList?.contains(e.target)) {
            setDropdownActive((prev) => !prev);
        }
    };
    // Clicking on select box for disable and loading
    const insideSelectClickHandler = () => {
        searchable && !disabled && userInputRef.current?.focus();
        disabled && setDropdownActive(false)
    }
    // Group Options
    function MoldGroupOptions(group: ObjI[]) {
        return group.map((option) => {
            const isSelectedOption = selectedOptions.includes(
            ...
            return (
                <li
                    className={`inte-Select__Select--Item inte-Select__Select--Item-List ${isSelectedOption
                        ? !multiSelect
                    ...
                    key={option.value}
                    value={option.value}
                    onClick={() => dropdownItemClickHandler(option)}
                >
                    {multiSelect && (
                        <span
                            style={{ visibility: isSelectedOption ? "visible" : "hidden" }}
                        >
                            <Check size="20" color="#413BBC" />
                        </span>
                    )}
                    <TextStyles
                        utility={
        ...
        return (
            ...
            >
                <FlexLayout halign="fill">
                    <TextStyles
                        utility="inte-Select__Select--ItemHeading"
                        textcolor="light"
                    >
                        {item.label}
                    </TextStyles>
    ...
    const renderSimpleData = (item: any, index: any) => {
        ...
        return (
            <li
                ...
                key={`${item.label}.${item.value}.${index}`}
                onClick={() => dropdownItemClickHandler(item)}
            >
                {multiSelect && (
                    <span
                        style={{
    ...
    const selectDropdownCard = (opts = options) => {
        return (
            ...
            >
                {isLoading || loading ? (
                    <div className="inte-select-options__loading">
    ...
    const dropdownItemClickHandler = (item: ObjI, selected = selectedOptions) => {
        const selectingOption = `${item.label}.${item.value}`;
        multiSelect
            ? setSelectedOptions((prev) =>
                prev.includes(selectingOption)
                    ? prev.filter((data) => data !== selectingOption)
                    : [...prev, selectingOption]
            )
            : setSelectedOptions(selected.find((data) => data === selectingOption) ? [] : [selectingOption]);
    ...
    const inputKeyPressHandler = (e: any) => {
        e.key === "Backspace" && inputValue === "" && selectedOptions.length && setSelectedOptions((prev) => prev.filter((item, i) => i !== prev.length - 1)
        );
    };
    ...
    const memoizedEffect = useCallback(() => {
        ...
        userInputRef.current?.addEventListener("input", resizeInputBox);
    ...
    const memoizedScrollWindow = useCallback(() => {
        scrollWindow();
    ...
    const scrollWindow = () => {
        ...
        const portalDropdown = portalDropdownListRef.current.getBoundingClientRect();

        portalDropdownListRef.current.style.width = `${(selectBox.width - 2) / 10}rem`;
        portalDropdownListRef.current.style.left = `${(selectBox.left + 1) / 10}rem`;
        if (clientHeight - selectBox.bottom > portalDropdown.height) {
            portalDropdownListRef.current.style.top = `${(selectBox.bottom + 3) / 10}rem`;
        } else {
            portalDropdownListRef.current.style.top = `${(selectBox.top - portalDropdown.height - 3) / 10}rem`;
        }
    };
    ...
    return (
        ...
        >
            ...
            <div
                className={`inte-formElemet--Inner ${controlStates ? controlStatesVal : ""
                ...
                >
                    {!multiSelect ? (
                        <span className="inte__Select--Selected">
                            {searchable && (
                                <input
                                    className={`inte-select-box__input ${disabled ? "inte-select--Disabled" : ""
                                        }`}
                        ...
                        <div
                            className={`inte__Select--Selected_tags ${disabled
            ...
            {!dropdownActive && helperText && (
                ...
                >
                    ...
                    <span
                        style={{ color: `${controlStates === "Error" ? "#C4281C" : ""}` }}
                    >
                        {helperText}
                    </span>
                </span>
